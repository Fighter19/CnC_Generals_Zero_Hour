...
public:
	...
	virtual void		Evaluate_Derivative(CustomFloat time,Vector3 * set_val);
...
public:
	...
	virtual void		Set_Tangents(int i,CustomFloat in_tan,CustomFloat out_tan);
	virtual void		Get_Tangents(int i,CustomFloat * set_in,CustomFloat * set_out);
...
protected:
	...
	{
	public:
		CustomFloat			InTangent;
		CustomFloat			OutTangent;
		bool				operator == (const TangentsClass & that) { return ((InTangent == that.InTangent) && (OutTangent == that.OutTangent)); }
		bool				operator != (const TangentsClass & that) { return !TangentsClass::operator == (that); }
	};
