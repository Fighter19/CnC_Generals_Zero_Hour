...
public:
	...
	bool					Is_Done(void) override { return Node==NULL; }
	...
	*/
	...
	for (ite.First();!ite.Is_Done();ite.Next()) {
		...
		if (tex->Num_Refs() == 1) {
			...
			if (count==256) {
				for (auto & temp_texture : temp_textures) {
					TextureHash.Remove(temp_texture->Get_Texture_Name());
					temp_texture->Release_Ref();
				}
				count=0;
